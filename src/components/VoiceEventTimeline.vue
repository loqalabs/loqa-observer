    ...
    <div v-if="sortedEvents.length > 0">
      ...
      <div class="mt-8 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
          <div>
            <div class="text-2xl font-bold text-gray-900 dark:text-white">{{ sortedEvents.length }}</div>
...
const sortedEvents = computed(() => 
  props.events.slice().sort((a, b) => 
    new Date(b.timestamp) - new Date(a.timestamp)
  )
)

const uniquePucks = computed(() => 
  new Set(sortedEvents.value.map(e => e.puck_id)).size
)
