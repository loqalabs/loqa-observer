          ...
          <div class="space-y-6">
            ...
            <div v-if="activeTab === 'overview'" class="space-y-6">
              ...
              <div>
                ...
                <dl class="grid grid-cols-1 gap-x-4 gap-y-3 sm:grid-cols-2">
                  <div>
                    ...
                    <dd class="text-sm text-gray-900 dark:text-white">{{ skill.manifest.author }}</dd>
                  </div>
                  <div>
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">License</dt>
                    <dd class="text-sm text-gray-900 dark:text-white">{{ skill.manifest.license }}</dd>
                  </div>
                  <div>
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Trust Level</dt>
                    <dd class="text-sm text-gray-900 dark:text-white">
                      <span
                        class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                        :class="{
                          'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300': skill.manifest.trust_level === 'system',
                          'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300': skill.manifest.trust_level === 'verified',
                          'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300': skill.manifest.trust_level === 'community',
                          'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300': skill.manifest.trust_level === 'unknown'
                        }"
                      >
                  ...
                  <div>
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Sandbox Mode</dt>
                    <dd class="text-sm text-gray-900 dark:text-white">{{ skill.manifest.sandbox_mode || 'none' }}</dd>
                  </div>
                </dl>
              </div>
              ...
              <div>
                ...
                <dl class="grid grid-cols-1 gap-x-4 gap-y-3 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <dd class="text-sm text-gray-900 dark:text-white">{{ formatDate(skill.loaded_at) }}</dd>
                  </div>
                  <div v-if="skill.last_used">
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Last Used</dt>
                    <dd class="text-sm text-gray-900 dark:text-white">{{ formatDate(skill.last_used) }}</dd>
                  </div>
                  <div>
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Usage Count</dt>
                    <dd class="text-sm text-gray-900 dark:text-white">{{ skill.status.usage_count || 0 }}</dd>
                  </div>
                  <div>
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Error Count</dt>
                    <dd class="text-sm text-gray-900 dark:text-white">{{ skill.error_count || 0 }}</dd>
                  </div>
                </dl>
              </div>
              ...
              <div>
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400">{{ skill.manifest.description }}</p>
              </div>

              <!-- Intent Patterns -->
              <div v-if="skill.manifest.intent_patterns && skill.manifest.intent_patterns.length">
                <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-3">Intent Patterns</h4>
                <div class="space-y-2">
                  <div
                  ...
                  >
                    <div>
                      <p class="text-sm font-medium text-gray-900 dark:text-white">{{ pattern.name }}</p>
            ...
            <div v-if="activeTab === 'config'" class="space-y-6">
              <div>
                <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-3">Skill Configuration</h4>
                <form @submit.prevent="saveConfig" class="space-y-4">
                  <!-- Enabled toggle -->
                  <div class="flex items-center justify-between">
                    <div>
                      <label class="text-sm font-medium text-gray-900 dark:text-white">Enabled</label>
                      <p class="text-sm text-gray-500 dark:text-gray-400">Whether this skill is active and can handle intents</p>
                    </div>
                    <button
                      type="button"
                  ...
                  <!-- Timeout -->
                  <div>
                    <label for="timeout" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Timeout</label>
                    <input
                      id="timeout"
                  ...
                  <!-- Max Retries -->
                  <div>
                    <label for="maxRetries" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Max Retries</label>
                    <input
                      id="maxRetries"
                  ...
                  <div v-if="Object.keys(configData.config || {}).length">
                    <h5 class="text-sm font-medium text-gray-900 dark:text-white mb-2">Custom Settings</h5>
                    <div class="space-y-3">
                      <div
            ...
            <div v-if="activeTab === 'permissions'" class="space-y-6">
              <div>
                <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-3">Required Permissions</h4>
                <div v-if="skill.manifest.permissions && skill.manifest.permissions.length" class="space-y-3">
                  <div
                  ...
                  >
                    <div class="flex-1">
                      <p class="text-sm font-medium text-gray-900 dark:text-white">{{ permission.type }}</p>
                      ...
                      <div v-if="permission.actions && permission.actions.length" class="mt-2">
                        <p class="text-xs text-gray-500 dark:text-gray-400">Actions: {{ permission.actions.join(', ') }}</p>
                      </div>
                    </div>
                    <span class="ml-4 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300">
                      Granted
                    </span>
                  </div>
                </div>
                <p v-else class="text-sm text-gray-500 dark:text-gray-400">This skill doesn't require any special permissions.</p>
              </div>
            </div>
          </div>
        </div>
